...
class PostProvider extends ChangeNotifier {
  ...
  Future<void> fetchAll({int? userId, String? search}) async {
    _setLoading(true);
    try {
      ...
      final response = await _api.get('/posts', queryParameters: queryParameters.isNotEmpty ? queryParameters : null);
...
Future<PostModel> fetchById(int id, {int? userId}) async {
  final post = await _api.get('/posts/$id'); // ou via PostService se preferir
...
Future<List<PostModel>> fetchByStoreId(int storeId, {int? userId}) async {
  final response = await _api.get('/posts?storeId=$storeId${userId != null ? '&userId=$userId' : ''}');
...
Future<PostModel> addComment({
  ...
  final result = await _interactions.addComment(postId: post.id, userId: userId, content: content);
